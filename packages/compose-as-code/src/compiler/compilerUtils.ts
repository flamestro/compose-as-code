...
export const compileObject = (obj: object, baseIndentationDepth: number) => {
    ...
    Object.keys(obj).forEach(key => {
        ...
        if (typeof value === 'number') {
            ...
            result += `${indent(baseIndentationDepth)}${key}: ${compileList(value, baseIndentationDepth + 1, { asSingleLineList: true })}\n`;
        } else {
            result += `${indent(baseIndentationDepth)}${key}: ${value}\n`;
        }
    });
...
export const compileList = (list: string[], baseIndentationDepth: number, options?: { asSingleLineList?: boolean }) => {
    let result = '';
