...
const compileService = (service: Service) => {
    ...
    if (service.deviceReadBps) {
        ...
        serviceTextBlock += compileKeyValuePair(
            'path',
            service.deviceReadBps.path,
            baseIndentation + 2
        );
        serviceTextBlock += compileKeyValuePair(
            'rate',
            service.deviceReadBps.rate,
            baseIndentation + 2
        );
    }
    if (service.deviceWriteBps) {
        ...
        serviceTextBlock += compileKeyValuePair(
            'path',
            service.deviceWriteBps.path,
            baseIndentation + 2
        );
        serviceTextBlock += compileKeyValuePair(
            'rate',
            service.deviceWriteBps.rate,
            baseIndentation + 2
        );
    }
    if (service.deviceReadIops) {
        ...
        serviceTextBlock += compileKeyValuePair(
            'path',
            service.deviceReadIops.path,
            baseIndentation + 2
        );
        serviceTextBlock += compileKeyValuePair(
            'rate',
            service.deviceReadIops.rate,
            baseIndentation + 2
        );
    }
    if (service.deviceWriteIops) {
        ...
        serviceTextBlock += compileKeyValuePair(
            'path',
            service.deviceWriteIops.path,
            baseIndentation + 2
        );
        serviceTextBlock += compileKeyValuePair(
            'rate',
            service.deviceWriteIops.rate,
            baseIndentation + 2
        );
    }
    if (service.weightDevice) {
        ...
        serviceTextBlock += compileKeyValuePair(
            'path',
            service.weightDevice.path,
            baseIndentation + 2
        );
        serviceTextBlock += compileKeyValuePair(
            'weight',
            service.weightDevice.weight,
            baseIndentation + 2
        );
    }
    if (service.weight) {
    ...
    if (service.deploy) {
        ...
        if (
            ...
            Object.keys(service.deploy.labels).length > 0
        ) {
            serviceTextBlock += compileKeyValuePair(
                'labels',
                '',
            ...
            serviceTextBlock += compileObject(
                service.deploy.labels,
                baseIndentation + 3
            );
        }
    ...
    if (service.command) {
        if (typeof service.command === 'string') {
            ...
            serviceTextBlock += compileKeyValuePair('command', '', baseIndentation + 1);
    ...
    if (service.dependsOn && service.dependsOn.length > 0) {
        ...
        serviceTextBlock += compileObjectListWithId(
            service.dependsOn,
            baseIndentation + 2
        );
    }
    ...
    if (service.volumes && service.volumes.length > 0) {
        ...
        serviceTextBlock += compileServiceVolumes(
            service.volumes,
            baseIndentation + 2
        );
    }
